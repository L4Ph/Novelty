...
アプリは手動でのみオン・オフできる「オフラインモード」を提供する。

- オフラインモードON時:
  - 新規のネットワーク通信を行わない。
  - ダウンロード済み・キャッシュ済みのコンテンツのみを利用する。
  - 検索・ランキング・一括ダウンロードなど、通信を要するUIは操作時に説明を表示し無効化する。
- オフラインモードOFF時:
  - 通常通りネットワークから最新データを取得する。
  - 取得失敗時はキャッシュがあればキャッシュを表示し、フォールバックイベントを通知する。

この設定は `AppSettings` の `isOfflineMode` として永続化され、状態は設定画面のトグルでのみ示される（常時表示のヘッダーバナーは表示しない）。
