...
class ApiService {
  ...
  static List<dynamic> _parseJson(List<int> bytes) {
    try {
      ...
      if (kDebugMode) {
        ...
        print('First 200 chars: ${decoded.length > 200 ? decoded.substring(0, 200) : decoded}');
      }
  ...
  Future<List<RankingResponse>> fetchRankingAndDetails(
    ...
    try {
      ...
      if (kDebugMode) {
        print('Successfully fetched ranking data, count: ${rankingData.length}');
      }
    } on Exception catch (e) {
      if (kDebugMode) {
      ...
      if (rankingType == 'd') {
        ...
        final previousDate = '${previousDay.year}${_twoDigits(previousDay.month)}${_twoDigits(previousDay.day)}';
        final fallbackUrl = 'https://api.syosetu.com/rank/rankget/?rtype=$previousDate-$rankingType&out=json&gzip=5';
        
        try {
          rankingData = await _fetchData(fallbackUrl);
          if (kDebugMode) {
            print('Successfully fetched ranking data from previous day, count: ${rankingData.length}');
          }
        } on Exception catch (fallbackError) {
          if (kDebugMode) {
            print('Failed to fetch ranking data from previous day: $fallbackError');
          }
          return [];
        }
      } else {
    ...
    for (final rankItem in rankingData) {
      ...
      try {
        if (novelDetails.containsKey(ncode)) {
          final details = Map<String, dynamic>.from(novelDetails[ncode] as Map<String, dynamic>);
  ...
  Future<List<RankingResponse>> _fetchAllTimeRanking() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('Successfully fetched all-time ranking, count: ${results.length}');
      }
