...
class DatabaseMigrationService {
  ...
  Future<void> _migrateNovels() async {
    final novels = await _oldDatabaseService.getNovelsForMigration();
      ...
      await _newDatabase.insertNovel(
        NovelsCompanion(
          ...
          generalFirstup: Value(
            int.tryParse((novel['general_firstup'] as String?) ?? ''),
          ),
          generalLastup: Value(
            int.tryParse((novel['general_lastup'] as String?) ?? ''),
          ),
  ...
  Future<void> _migrateHistory() async {
    final historyItems = await _oldDatabaseService.getHistoryForMigration();
  ...
  Future<void> deleteOldDatabase() async {
    ...
    final dbPath = join(appDocDir.path, 'novelty.db');
