...
@JsonKey(name: 'userid') int? get userId;/// 全掲���エピソード数。
///
/// 短編の場合は 1。
...
@useResult
$Res call({
 int? rank, int? pt,@JsonKey(name: 'all_point') int? allPoint, String ncode, String? title,@JsonKey(name: 'novel_type') int? novelType, int? end, int? genre, String? writer, String? story,@JsonKey(name: 'userid') int? userId,@JsonKey(name: 'general_all_no') int? generalAllNo, String? keyword
});
...
class _RankingResponse implements RankingResponse {
  const _RankingResponse({this.rank, this.pt, @JsonKey(name: 'all_point') this.allPoint, required this.ncode, this.title, @JsonKey(name: 'novel_type') this.novelType, this.end, this.genre, this.writer, this.story, @JsonKey(name: 'userid') this.userId, @JsonKey(name: 'general_all_no') this.generalAllNo, this.keyword});
